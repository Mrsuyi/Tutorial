...
@interface UrlViewController ()<NSURLSessionDelegate, NSURLSessionDataDelegate> {
}
@end
...
- (void)viewDidLoad {
  ...
  [constraints addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-20-[btn]-10-[tv(>=200)]-10-|" options:0 metrics:nil views:views]];
  [constraints addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-20-[btn]" options:NSLayoutFormatAlignAllCenterY metrics:nil views:views]];
  [constraints addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-5-[tv]-5-|" options:NSLayoutFormatAlignAllCenterY metrics:nil views:views]];
  ...
  _normalSession = [NSURLSession sessionWithConfiguration:config delegate:self delegateQueue:nil];
}

- (void)onBtnClicked {
  NSLog(@"shit");
  NSURL* url = [NSURL URLWithString:@"https://www.google.com"];
  NSURLSessionDataTask* task = [_normalSession dataTaskWithURL:url completionHandler:^(NSData* data, NSURLResponse* response, NSError* error) {
    if (!data) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      if (response.textEncodingName) {
        CFStringEncoding c = CFStringConvertIANACharSetNameToEncoding((CFStringRef)response.textEncodingName);
